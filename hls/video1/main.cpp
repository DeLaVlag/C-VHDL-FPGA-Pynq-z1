...
//		src >> streamIn;
		...
		for (int wRows = 0; wRows < KERNEL_SIZE; wRows++)
		{
			for (int wCols = 0; wCols < KERNEL_SIZE; wCols++)
			{
				...
				uint24 val = (uint24)lb.getval(wRows,wCols+slidefactor);

				// kernel * linebufcontent and place in a 3x3 window
				val = (uint24)kernel[(wRows*KERNEL_SIZE) + wCols ] * val;
				//if(val<0)val=0;
...
uint24 pixelSummer(hls::Window<KERNEL_SIZE,KERNEL_SIZE,uint24> *resultfromlinesliding)
{
	uint24 sum = 0;
	...
	for (int rows = 0; rows < KERNEL_SIZE; rows++)
	{
		...
		{
			sum = sum + (uint24)resultfromlinesliding->getval(rows,cols);
		}
	}
	return sum;
}
